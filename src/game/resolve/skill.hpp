 ...
 * @note 与装备可选触发同范式——结算点直接调用决策接缝（不订阅事件）；无武将或
 *       无对应技能时首行短路，保证无名座位逐字节零行为。
 * @note 触发钩子内的结算须有重入保护：本期反馈只移牌、不再次造成伤害；后续出现
 *       会造伤的触发技须显式加守卫。
 */
...
namespace tkw
{
    namespace game
    {
         ...
         * @param victim 受到伤害的实体 id（技能拥有者）。
         * @param source 伤害来源实体 id（空串 = 无来源，如闪电）。
         ...
         * @param applied 本次伤害实际扣减量；<= 0 表示未实际受伤，不触发。

 ...
 * @note 只描述「视口在哪里、按键是否被日志接管」，不含终端读写与渲染；由 FTXUI
 *       壳层读取 ratio 放进视口，并把按键映射成 LogKey 后调用本层判定。
 */
...
namespace tkw
{
    namespace tui
    {
         ...
         * @note 非跟随时 anchor 为绝对行下标（0=最早行），新日志到达不改变窗口；
         *       跟随时 anchor 无意义。行数变化由调用方以最新 total 传回各方法。
         */
        struct LogScroll
        {
             ...
             * @param total 当前日志总行数；0 时锚到 0 并保持跟随。
             */
             ...
             * @param total 当前日志总行数；顶部恒为下标 0，参数保留与 to_tail 对称。
             */
         ...
         * @param key         本次按键映射。
         * @param input_empty 命令输入缓冲是否为空。
         * @param scroll      日志滚动状态（就地更新）。
         * @param total       当前日志总行数。
         * @return true=已消费（事件不再下发）；false=落回命令输入。
         * @note PageUp/PageDown 无输入门控；Home/End 仅命令输入为空时消费，避免
         *       抢占单行编辑的光标键。
         */

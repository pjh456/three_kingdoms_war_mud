/**
 * @file app.hpp
 * @brief TUI 应用壳：FTXUI 适配层，持控制器与命令输入框，渲染四面板。
...
namespace tkw
{
    namespace tui
    {
        /**
         * @class App
         * @brief TUI 壳：把 FTXUI 组件、屏幕回送接缝与命令输入接到 Controller。
         */
        class App
        {
        public:
            ...
            /**
             * @brief 组装根组件：命令输入 + 渲染器 + Esc/Ctrl-C 退出。
             * @param screen 全屏屏幕；注入 Post 与退出回调都由其承载。
             * @note 退出由控制器 request_quit 统一处理（取消 → join → 存档 →
             *       Exit），Esc/Ctrl-C 不再直接调用 screen.Exit()。
             */
